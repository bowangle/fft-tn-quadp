...
void test_build(const std::string& name, int r, int chi, int n_points = 10)
{
    ...
    // 4. Build TT from W
    auto tt = magic_tensor_qft::build_mpo<ComplexT>(W, chi);
    std::cout << "  TT nBit = " << tt.get_size() << "\n";
    std::cout << "  TT chi  = " << tt.get_chi() << "\n";
    ...
    auto points = tt.generate_points(n_points);
    auto vals = tt.eval_list(points);
    ...
    auto tt2 = magic_tensor_qft::build_qft_mpo_magic<ComplexT>(r, chi);
    std::cout << "  build_qft_mpo_magic: nBit=" << tt2.get_size()
              << " chi=" << tt2.get_chi() << "\n";
}
